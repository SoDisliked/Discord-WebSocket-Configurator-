...
namespace DislikedWebSocketConfiguration
{
    public class RequestParser
    {
        const string pattern = @"^(?<method>[^\s]+)\s(?<path>[^\s]+)\sHTTP\/1\.1\r\n" + // request line
                               ...
                               @"\r\n" + //newline
                               @"(?<body>.+)?";
        const string FlashSocketPolicyRequestParser = @"^[<]policy-file-request\s*[/][>]";
        ...
        public statitc WebSocketHttpRequest Parse(byte[] bytes)
        {
        ...
        {
            // Check websocket request handler if working
            ...
            if (!match.Success)
            {
                ...
                if (match.Success)
                {
                    // It's a flash socket policy request, return command
